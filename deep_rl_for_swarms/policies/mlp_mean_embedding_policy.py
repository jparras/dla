...
class MlpPolicy(object):
    ...
    def _init(self, ob_space, ac_space, hid_size, feat_size, gaussian_fixed_var=True):
        ...
        mean_emb = ob_space.dim_mean_embs  # Fixed in the agent
        nr_rec_obs = mean_emb[0]
        dim_rec_obs = mean_emb[1]
        dim_flat_obs = ob_space.dim_flat_o  # Fixed in the agent
        ...
        flat_obs_input_layer = tf.slice(ob, [0, 0], [-1, nr_rec_obs * dim_rec_obs]) # Info that goes to the mean embedding
        flat_feature_input_layer = tf.slice(ob, [0, nr_rec_obs * dim_rec_obs], [-1, dim_flat_obs]) # Local, private info
        ...
        with tf.variable_scope('vf'): # Value NN
            ...
            last_out = tf.concat([me_v.me_out, flat_feature_input_layer], axis=1) # Size: 64 (mean embeding output) + 5 (local data)
