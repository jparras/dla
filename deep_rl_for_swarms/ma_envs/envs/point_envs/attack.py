...
class AttackEnv(gym.Env):
    ...
    def __init__(self, nr_agents=5,  # Agents in the swarm
                 ...
                 obs_mode='sum_obs', # Observation mode
        ...
        if self.attack_mode == 'phy':
            ...
            self.action_size = 2  # Discrete actions: wether to transmit or not
        else:
    ...
    def reset(self): # Method to reset the environment
        ...
        self.distance_matrix = U.get_distance_matrix(self.agent_pos, torus=False, world_size=self.world_size,
                                                     add_to_diagonal=-1)
        self.rewards_hist = []
        if self.attack_mode == 'phy':
    ...
    def step_mac(self, actions):
        ...
        # Update backoff of the attackers
        if self.nr_agents > 0:
    ...
    def step(self, actions):
        ...
        if self.attack_mode == 'phy':
            ...
            reward, done = self.step_mac(actions)
            self.timestep += 1  # Increase timestep
            ...
            if np.sum(self.banned[0:self.nr_agents]) == self.nr_agents:
                while not done:
                    r_mac, done = self.step_mac(np.zeros_like(actions))
                    self.timestep += 1
                    reward += r_mac
        else:
    ...
    def phy_plot(self, save, show, title):
        t = np.arange(self.timestep)  # Time values
        '''
        ...
        for s in range(self.nr_agents):
            if self.banned[s]:
                plt.plot(t, self.energy_hist[s, 0: self.timestep] / self.energy_norm, 'r')
            else:
                plt.plot(t, self.energy_hist[s, 0: self.timestep] / self.energy_norm, 'g')
        plt.plot(t, self.energy_hist[self.nr_agents:, 0: self.timestep].T / self.energy_norm, 'b')
        if save:
            from tikzplotlib import save as tikz_save
            if title is None:
                name = str(self.nr_agents) + '_' + str(self.nr_agents_total) + '_' + str(np.random.randint(low=0, high=1024)) + '_phy'
            else:
                name = title
            tikz_save(name + '.tex') #, figureheight='\\figureheight', figurewidth='\\figurewidth')
        if title is None:
            pass
        else:
            plt.title(title)
    ...
    def run_baseline(self, policy_type, num_episodes=50):
        ...
        elif policy_type == 'grid' or policy_type == 'always_attack' or policy_type == 'never_attack':
    ...
    def obtain_baseline_policy(self, policy_type):
        ...
        elif policy_type == 'always_attack':
            return np.ones(1)
        elif policy_type == 'never_attack':
            return np.zeros(1)
        else:
...
class random_pol:  # This class is only created for debugging
...
if __name__ == '__main__':
    n_ag = 11
    env = AttackEnv(nr_agents=n_ag, nr_agents_total=21, obs_mode='sum_obs_multi', attack_mode="phy",
                    obs_radius=5000, world_size=1000, phy_agent_memory=5, lambda_phy=0.5,
                    K=5, L=1000, lambda_mac=0.5, timesteps_limit=50, sum_rwd=True)
